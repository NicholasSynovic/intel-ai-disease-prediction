...
# Original code from
# https://github.com/oneapi-src/disease-prediction/blob/main/src/run_training.py
...
def main(flags):
    ...
    logger.debug("Reading in the data from %s", flags.data_dir)
    path = pathlib.Path(flags.data_dir)

    try:
        train_dataset = read_and_preprocess_data(
            path / "Training.csv", tokenizer, max_length=flags.seq_length
        )
        test_dataset = read_and_preprocess_data(
            path / "Testing.csv", tokenizer, max_length=flags.seq_length
        )
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        "emilyalsentzer/Bio_ClinicalBERT", num_labels=41
    )
    ...
    for _, (batch, labels) in enumerate(val_loader):
        ...
        pred = model(input_ids=ids, attention_mask=mask, token_type_ids=token_type_ids)
    ...
    logger.info(
        "=======> Test Accuracy : %.2f", accuracy_score(test_preds, test_labels)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch_size", default=30, type=int, help="Batch size to use when training."
    )
    ...
    parser.add_argument(
        "--grad_norm", default=10.0, type=float, help="Gradient clipping cutoff."
    )
    ...
    parser.add_argument(
        "--bf16", default=False, action="store_true", help="Enable bf16 training"
    )

    FLAGS = parser.parse_args()

    if not (
        (pathlib.Path(FLAGS.data_dir) / "Training.csv").exists()
        and (pathlib.Path(FLAGS.data_dir) / "Testing.csv").exists()
    ):
